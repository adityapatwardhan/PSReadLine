...
namespace Microsoft.PowerShell
{
    public partial class PSConsoleReadLine
    {
        ...
        /// Some summary
        /// </summary>
        public static void DynamicHelpBlock(ConsoleKeyInfo? key = null, object arg = null)
        {
            bool isF1 = key?.Key == ConsoleKey.F1;
            _singleton.DynamicHelpBlockImpl(isF1);
        }
        ...
        private void DynamicHelpBlockImpl(bool isFullHelp)
        {
            ...
            {
                System.Management.Automation.PowerShell ps = System.Management.Automation.PowerShell.Create(RunspaceMode.CurrentRunspace);
                PSObject paramHelp = ps.AddScript($"(Get-Help -Name {commandName} -Parameter {parameterName})").Invoke<PSObject>().FirstOrDefault();
        ...
        private void WriteToAlternateScreenBuffer(string helpContent, string parameterName)
        {
            ...
            string endAltBuffer = $"{c}[?1049l";
            string reverseColorStart = $"{c}[7m";
            string reverseColorEnd = $"{c}[0m";

            var contentAsArray = helpContent.Split(Environment.NewLine);
        ...
        private int GetMultilineOffset(string[] contentAsArray, int startLine)
        {
            int bufferHeight = Math.Min(Console.WindowHeight,Console.BufferHeight);
